...
<table style="width: 100%; border-collapse: collapse; font-family: sans-serif; border: 1px solid #333; margin-top: 20px;">
  <thead>
    <tr style="background-color: #fff; border-bottom: 1px solid #333;">
      ...
      <th style="padding: 12px; text-align: center; width: 20%; font-weight: bold; color: #333; text-transform: uppercase; font-size: 14px;">DETAILS</th>
    </tr>
  </thead>
  <tbody>
...
with engine.connect() as conn:
    conn.execute(text('UPDATE department_sections SET content = :content WHERE id = 44'), {"content": new_content})
    conn.commit()
    print("Syllabus style updated with borders to match screenshot.")
